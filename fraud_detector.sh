...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -r)          check_root; init_logger "$LOG_CUSTOM_DIR"; restore_logs; exit 0 ;;

        -l)
            [[ -z "$2" ]] && die "$E_MISSING_PARAM" "-l requiert un chemin de répertoire"    
            LOG_CUSTOM_DIR="$2"; shift 2 ;;     
...
if [[ -n "$INTERNAL_MODE" ]]; then
    # Initialiser le logger sans affichage c'est a dire en arriere plan 
    init_logger ""
    ...
    case "$INTERNAL_MODE" in
        ...
        frequency)   detect_frequency_anomaly "$CSV_DATA" ;;
        behavior)    detect_behavior_change "$CSV_DATA" ;;
        structuring) detect_structuring "$CSV_DATA" ;;
        switching)   detect_account_switching "$CSV_DATA" ;;
    esac
    exit "$ALERT_COUNT"
fi
...
[[ -z "$CSV_DATA" ]] && die "$E_INVALID_CSV" "Aucune donnée valide (ou utilisateur '$FILTER_USER' introuvable)."

NB_LINES=$(echo "$CSV_DATA" | wc -l | tr -d ' ')
log_info "Démarrage fraud_detector.sh — mode: $MODE — fichier: $CSV_FILE ($NB_LINES lignes)"

[[ -n "$FILTER_USER" ]] && echo -e "${BLUE}[INFO]${RESET} Filtre: ${BOLD}$FILTER_USER${RESET}"
[[ -n "$EXPORT_FILE" ]] && mkdir -p "${SCRIPT_DIR}/reports"

# ── Statistiques (--stats) ───────────────────────────────────
$DO_STATS && show_stats "$CSV_DATA"
